...
function App() {
  const [files, setFiles] = useState<CImage[]>([]);
  ...
  const listItems = files.map((path) => <li key={path.id}>{path.path}</li>);
  ...
  return (
    <main className="w-screen h-screen bg-gray-200 text-center">
      ...
      <button className="bg-white border border-gray-300 rounded-md px-4 py-2 m-2" onClick={open_files}>
        Open
      </button>
